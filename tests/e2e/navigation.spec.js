...
test.describe("Navigation and Cross-Page Tests", () => {
  test.describe("Homepage Navigation", () => {
    ...
    test("[smoke] navigates to resume page from homepage", async ({ page }) => {
  ...
  test.describe("Resume Page Navigation", () => {
    ...
    test("[smoke] navigates back to homepage from resume", async ({ page }) => {
  ...
  test.describe("Keyboard Navigation", () => {
    // Skip entire Keyboard Navigation suite on WebKit where Tab traversal is unreliable in automation
    test.beforeAll(async ({ browserName }) => {
      if (browserName === "webkit") {
        test.skip(true, "Skipping Keyboard Navigation tests on WebKit");
      }
    });

    test("tab navigation works across pages", async ({ page, browserName }) => {
      // WebKit/iOS requires OS-level Full Keyboard Access for Tab traversal to links/buttons
      // which is not controllable in automation; skip this Tab traversal assertion there.
      if (browserName === "webkit") {
        test.skip(true, "Tab key focus traversal is not reliable in WebKit/iOS automation");
      }

      // Test homepage tab navigation
      await page.goto("/");

      // Ensure page has focus before tabbing (needed for WebKit/Safari)
      await page.locator("body").click();

      // Tab to the first interactive element (robust across browsers)
      ...
      for (let i = 0; i < 10; i++) {
        ...
        if (allowedTags.includes(focusedElement)) break;
      }
      expect(allowedTags.includes(focusedElement)).toBeTruthy();
